...
function MainMenuMicroButtonMixin:bagbuttons_setup()
	...
	MainMenuBarBackpackButton:SetClearPoint('BOTTOMRIGHT', HelpMicroButton, 'BOTTOMRIGHT', -5, 36)
	...
	KeyRingButton.IconBorder:Hide()
	KeyRingButton.Icon:Hide()
...
local function setMicroButtonTexture(button)
	local buttonName = button:GetName():gsub('MicroButton', '')
	local name = strlower(buttonName);
	button:texture_strip()
	button:SetSize(14, 19)
...
microEventFrame:SetScript("OnEvent", updateMicroButtonsTextures)
...
microEventFrame:RegisterEvent("PLAYER_LEVEL_UP")

if microEventFrame.RegisterCustomEvent ~= nil then
	microEventFrame:RegisterCustomEvent("LF_GUILD_RECRUIT_LIST_CHANGED")
	microEventFrame:RegisterCustomEvent("LF_GUILD_MEMBERSHIP_LIST_CHANGED")
end

_LoadMicroButtonTextures = _G.LoadMicroButtonTextures
_G.LoadMicroButtonTextures = function(self, ...)
	_LoadMicroButtonTextures(self, ...)
	setMicroButtonTexture(self)
	GuildMicroButton.Spinner:SetAlpha(0)
end

_UpdateMicroButtons = _G.UpdateMicroButtons
_G.UpdateMicroButtons = function(...)
	_UpdateMicroButtons(...)
	updateMicroButtonsTextures()
end
