...
local nobar = config.additional.pet.y_position;
local exOffs = config.additional.pet.leftbar_offset;
local exOffs2 = config.additional.pet.rightbar_offset;
...
function anchor:petbar_update()
	...
	if not InCombatLockdown() and not UnitAffectingCombat('player') then
		...
		elseif rightbar then
			...
			self:SetPoint("TOPLEFT", pUiMainBar,'TOPLEFT', offsetX - 7, nobar);
		end
	end
end
...
local btnsize = config.additional.pet.size;
...
local function petbutton_position()
	...
	for index=10, 1, -1 do
		...
		if index == 10 then
			...
			button:SetPoint('RIGHT', _G['PetActionButton'..index+1], 'LEFT', -space, 0);
		end
		button:Show();
